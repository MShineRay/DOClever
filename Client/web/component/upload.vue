<template>
  <el-dialog title="上传" width="50%" ref="box" :modal="hud" :visible.sync="showDialog" append-to-body
             :close-on-press-escape="false" :close-on-click-modal="false" :show-close="!uploadPending">
    <el-row class="row" style="height: 200px;border: 1px lightgray dashed;text-align: center" draggable="true"
            id="dragFile" @dragenter.native.self="dragEnter($event)" @dragleave.native.self="dragLeave($event)"
            @dragover.native.self="dragOver($event)" @drop.native="drop($event)">
      <i class="el-icon-upload" style="font-size: 60px;color: darkgray;margin-top: 30px"></i>
      <div>
        拖动文件到这里
      </div>
      <el-progress :text-inside="true" :stroke-width="18" :percentage="per" status="success"
                   style="width: 80%;margin:20px auto 0 auto"></el-progress>
    </el-row>
  </el-dialog>
</template>
