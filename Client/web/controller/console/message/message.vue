<template>
  <el-row class="row" style="margin-top: 10px" id="message">
    <el-button icon="el-icon-refresh" size="mini" type="text"
               style="position: absolute;top: 5px;right: 20px;z-index: 1000;font-size: 14px" title="刷新"
               @click="refresh">
    </el-button>
    <el-button icon="el-icon-delete" size="mini" type="text"
               style="position: absolute;top: 5px;right: 50px;z-index: 1000;font-size: 14px;color: red" title="清空"
               @click="clear" v-if="type==1 && arr.length>0">
    </el-button>
    <el-tabs v-model="type" style="height: 50px;" id="interfaceTab">
      <el-tab-pane :name="0">
        ...
        <el-row class="row" style="padding: 10px 10px 50px 10px;overflow-y: auto;height: calc(100vh - 115px)">
          ...
          <expand
            style="background-color: white;border-radius: 5px;box-shadow: 0 2px 4px 0 rgba(0,0,0,.12), 0 0 6px 0 rgba(0,0,0,.04);margin-top: 15px;">
            ...
            <table class="table-hover" style="width: 100%">
              <template v-for="item in $store.state.arrTeamPullProject">
                <tr class="row" style="height: 40px;vertical-align: middle;text-align: center">
                  <td style="width: 70%">
                    团队<span
                    style="color: #00adef">{{item.from ? item.from.name : ""}}</span>邀请您的{{item.type == 1 ? "接口" : "文档"}}项目{{item.to.name}}加入
                  </td>
                  <td style="width: 30%">
                    <template v-if="item.handle==0">
            ...
            <table class="table-hover" style="width: 100%">
              <template v-for="item in $store.state.arrUserApplyTeam">
                <tr class="row" style="height: 40px;text-align: center;vertical-align: middle">
                  <td style="width: 60%">
                    用户<span
                    style="color: #00adef">{{item.from ? item.from.name : ""}}</span>申请加入您的团队{{item.to.name}}<br>
      ...
      <el-tab-pane :name="1" label="已处理">
        <el-row class="row" v-scroll="loadMore" style="height: calc(100vh - 115px);overflow-y: auto;padding: 10px">
          <el-row class="row"
                  style="background-color: white;padding: 10px;border-radius: 5px;box-shadow: 0 2px 4px 0 rgba(0,0,0,.12), 0 0 6px 0 rgba(0,0,0,.04);"
                  v-if="arr.length>0">
            <template v-for="item in arr">
              <el-row class="row message-hover" :key="item._id" style="border-bottom: 1px lightgray solid">
                ...
                <el-row class="row" style="color: gray;height: 30px;line-height: 30px">
                  {{item.createdAt}}&nbsp;&nbsp;&nbsp;
                  <el-button type="text" size="mini" style="color:#FF4949" icon="el-icon-delete"
                             @click="remove(item,index)" titile="删除">
                  </el-button>
                </el-row>
              </el-row>
            </template>
            <el-row class="row" :loading="loading" style="height: 30px;line-height: 30px;text-align: center;color:gray"
                    v-if="!finish">
            </el-row>
          </el-row>
        </el-row>
