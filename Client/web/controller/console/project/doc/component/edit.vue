<template>
  <el-dialog ref="box" :fullscreen="true" :visible.sync="showDialog" append-to-body id="editDoc" :before-close="close"
             @click.native="menu=0">
    ...
    <el-row style="background-color: white;height: 100%;border-top: 1px lightgray solid;font-size: 14px" class="row">
      <el-col class="col" :span="4" style="border-right: 1px lightgray solid">
        <el-row class="row"
                style="height: 30px;line-height: 30px;background-color: rgb(244,241,244);text-align: left;padding-left: 5px">
          目录
        ...
        <el-row class="row"
                style="height: 30px;line-height: 30px;background-color: rgb(244,241,244);text-align: center">
      ...
      <el-col class="col" :span="10" style="border-right: 1px lightgray solid">
        <textarea
          style="width: 100%;height: calc(100% - 1px);resize: none;box-sizing: border-box;padding: 10px 10px calc(100vh - 18px * 1.5) 10px;margin: 0px;border: 0px;outline: none;font-size: 18px;line-height: 1.5;"
          placeholder="请填入文档内容，支持markdown语法，编辑框内右键弹出菜单，可直接粘贴截图！" @contextmenu="contextMenu($event)"
          @mouseup="recordStart($event)" v-show="selDoc" v-model="content" @scroll.stop.prevent="scroll($event)"
          id="inputContent" @input="input=1" spellcheck="false" @paste="paste($event)"></textarea>
      </el-col>
      <el-col class="col docPreview" :span="10" v-html="preview"
              style="overflow-y: auto;word-break: break-all;padding: 0 10px 0 10px;line-height: 1.5" id="preview">

      </el-col>
    </el-row>
    <el-row class="row"
            style="width: 100px;height: 90px;position: absolute;top:0;left:0;border: 1px lightgray solid;background-color: white;text-align: center;cursor: pointer"
            v-show="menu" id="menu">
      <el-row class="row menuHover" style="height: 30px;line-height:30px;border-bottom: 1px lightgray solid"
              @click.native="insertImg">
        <i class="el-icon-picture"></i>&nbsp;上传图片
      </el-row>
      <el-row class="row menuHover" style="height: 30px;line-height:30px;border-bottom: 1px lightgray solid"
              @click.native="insertFile">
...
<script>
  ...
  module.exports = {
    ...
    created: function () {
      var _this = this;
      this.socket = window.io.connect(location.host);
