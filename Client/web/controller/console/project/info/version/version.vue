      ...
      <el-row class="row" style="padding: 10px 20px 10px 20px">
        <el-form ref="form" label-width="100px" label-position="top" v-if="session.versionId">
          ...
          <el-row class="row" style="margin-top: 10px">
            <el-button size="mini" type="primary" @click.prevent="saveVersion" :loading="savePending"
                       v-if="versionEditRole">
              保存
            </el-button>
            <el-button size="mini" type="danger" @click.prevent="rollVersion" :loading="rollPending"
                       v-if="versionRollRole">
              回滚
            </el-button>
            <el-button size="mini" type="danger" @click.prevent="removeVersion" :loading="removePending"
                       v-if="versionEditRole">
              删除
            </el-button>
        ...
        <el-row class="row" v-else>
          当前是主版本
          <el-button type="primary" size="mini" style="margin-left: 20px" @click="showVersion=true"
                     :loading="createPending" v-if="!session.versionId && versionEditRole">
            创建版本
          </el-button>
        </el-row>
      </el-row>
            ...
            <tr style="height: 40px;text-align: center;vertical-align: middle">
              ...
              <td style="width:20%">
                <el-button type="text" size="mini"
                           @click.native="editIndex=index,editId=item._id,editName=item.version,editDis=item.dis,showEditVersion=true"
                           v-if="versionEditRole">编辑
                </el-button>
                <el-button type="text" size="mini" @click.native="switchVersion(item,index)">切换</el-button>
                <el-button type="text" size="mini" @click.native="rollVersion(item,index)" style="color: red"
                           v-if="versionRollRole">回滚
                </el-button>
                <el-button type="text" size="mini" @click.native="removeVersion(item,index)" style="color: red"
                           v-if="versionEditRole">删除
                </el-button>
              </td>
            </tr>
          </template>
