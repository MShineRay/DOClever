<template>
  <el-row class="row" style="padding: 10px 10px 60px 10px;height: calc(100vh - 115px);overflow-y: auto;font-size: 14px"
          id="globalInfo">
    <expand class="row"
            style="border-radius: 5px;box-shadow: 0 2px 4px 0 rgba(0,0,0,.12), 0 0 6px 0 rgba(0,0,0,.04);background-color: white"
            id="projectBasicInfo">
          ...
          <el-row class="row">
            ...
            <el-col class="col" :span="10">
              <el-form-item label="简介">
                <el-tooltip class="item" effect="dark" :content="project.dis" placement="bottom"
                            :disabled="!project.dis">
          ...
          <el-row class="row">
            <el-col class="col" :span="10">
              <el-form-item label="公开">
                <el-switch v-model="project.public" active-color="#13ce66" inactive-color="lightgray" :active-value="1"
    ...
    <expand class="row"
            style="border-radius: 5px;box-shadow: 0 2px 4px 0 rgba(0,0,0,.12), 0 0 6px 0 rgba(0,0,0,.04);background-color: white;margin-top: 10px"
            id="statusEdit">
      <el-row class="row" style="height: 40px;line-height: 40px;color: #17b9e6" slot="title">
        修改项目成员
      </el-row>
    ...
    <expand class="row"
      ...
      <el-row class="row" style="padding: 10px 20px 10px 20px">
        ...
        Mock Js文件：<a href="/html/web/resource/other/net.js" target="_blank">net.js</a>（和内网测试是同一个文件，需要安装node环境，安装包点击下载：<a
        href="/html/web/resource/other/node.msi" target="_blank">window</a>&nbsp;&nbsp;<a
        href="/html/web/resource/other/node.pkg" target="_blank">mac</a>）<br>
      ...
      <template v-if="!session.teamId && manageRole">
        ...
        <el-row class="row" style="padding: 10px 20px 10px 20px">
          <el-form ref="form" label-width="100px" label-position="top">
                ...
                <el-form-item label="备注">
                  <el-tooltip class="item" effect="dark" :content="teamDis" placement="bottom" :disabled="!teamDis">
                    <el-input size="small" style="width: 90%" v-model="teamDis">
                      <i slot="suffix" class="el-input__icon el-icon-edit" @click="editTeamApply"
                         style="cursor: pointer"></i>
                    </el-input>
                  </el-tooltip>
            ...
            <el-button size="mini" type="primary" style="margin-top: 10px" @click.prevent="applyTeam"
    ...
    <expand class="row"
            style="border-radius: 5px;box-shadow: 0 2px 4px 0 rgba(0,0,0,.12), 0 0 6px 0 rgba(0,0,0,.04);background-color: white;margin-top: 10px"
            v-if="project.source && manageRole">
      <el-row class="row" style="height: 40px;line-height: 40px;color: #17b9e6" slot="title">
        更新工程
      </el-row>
      <el-row class="row" style="background-color: lightgray;height: 1px"></el-row>
      <el-row class="row" style="padding: 10px 20px 10px 20px">
        <el-row class="row" style="word-break: break-all;">
          ...
          <el-radio class="radio" :label="1" v-model="importType">JSON</el-radio>
          <br><br>
          <el-input size="small" v-model="project.source.url" style="width: 80%" placeholder="请输入更新的url地址"
                    v-if="importType==0"></el-input>
          <el-input size="small" v-model="importText" type="textarea" :rows="5" placeholder="请输入更新的JSON" v-else
                    style="width: 80%"></el-input>
        </el-row>
        <el-button size="mini" type="primary" style="margin-top: 10px" @click="updateProject" :loading="updateLoading">
          更新
        </el-button>
      </el-row>
    </expand>
