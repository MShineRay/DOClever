<template>
  <el-row class="row">
      ...
      <template v-for="(item,index) in arr">
        <tr :style="{textAlign: 'center',verticalAlign: 'middle',backgroundColor:item.enable?'white':'lightgray'}">
          <td style="width: 20%;text-align: center;vertical-align: middle;height: 50px">
            <el-input size="small" style="width: 90%;" placeholder="请填写参数名称" v-model.trim="item.name"
                      :disabled="!item.enable" @input="index==arr.length-1?add():''"></el-input>
          </td>
          ...
          <td
            style="width: 20%;text-align: center;vertical-align: middle;height: 50px;line-height: 50px;overflow: auto">
            <div style="width: 90%;display: inline-block;"
                 v-if="item.type==0 && item.value && (item.value.data.length>0 || item.value.status)">
              <el-autocomplete size="small" class="inline-input" v-model="item.selValue"
                               :fetch-suggestions="querySearch" placeholder="选择或者填入你的值" @mouseenter.native="focus(item)"
                               :disabled="!item.enable" custom style="width:100%" popper-class="my-autocomplete">
                <i class="el-icon-caret-bottom el-input__icon" slot="suffix" @click="showAutoComplete"
            ...
            <el-input size="small" style="width: 90%;" placeholder="请填写值" v-model="item.selValue"
                      v-else-if="item.type==0" custom></el-input>
          ...
          <td style="width: 10%;height: 50px;">
            <el-button type="text" size="mini" style="font-size: 13px" @click="encrypt(item)" v-if="item.type==0">
              {{(item.encrypt && item.encrypt.type) ? item.encrypt.type : "未加密"}}
            </el-button>
          </td>
          <td style="width: 5%">
            <el-button type="text" size="mini" style="font-size: 15px;" @click="toggleEnable(item,index)"><span
              :class="item.enable?'fa fa-eye-slash':'fa fa-eye'" :title="item.enable?'发送时不包含此字段':'发送时包含此字段'"></span>
            </el-button>
          </td>
          <td style="width: 5%">
            <el-button type="text" size="mini" icon="el-icon-close" style="color: red;font-size: 15px;"
                       @click="remove(index)" v-if="index!=arr.length-1"></el-button>
          </td>
        </tr>
      </template>
    </table>
    <el-row class="row" style="padding: 0 0 0 20px;" v-show="info.type==1 && info.rawType==2">
      ...
      <el-button type="primary" size="mini" style="margin-top: 10px;margin-left: 20px" @click="editJSON">
        {{editType == 0 ? "Edit JSON" : "Commit JSON"}}
      </el-button>
      <el-button type="primary" size="mini" style="margin-top: 10px;margin-left: 20px" @click="editType=0"
                 v-if="editType==1">Cancel Edit
      </el-button>
    </el-row>
    <el-row class="row" style="padding: 0 0 0 20px;" v-show="info.type==1 && info.rawType!=2">
      ...
      <el-row class="row" style="margin-top: 10px">
        <el-input size="small" type="textarea" :rows="3" style="width: 100%;height: 200px;" v-if="info.rawType==0"
                  id="bodyText" v-model="info.rawText" :placeholder="info.rawTextRemark?info.rawTextRemark:'请填入Raw内容'">
        </el-input>
