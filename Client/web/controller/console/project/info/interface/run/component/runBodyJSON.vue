        ...
        <tr style="text-align: center;vertical-align: middle;cursor: move;height: 50px;line-height: 50px"
          ...
          <td :style="{width: '30%',verticalAlign: 'middle',paddingLeft:level*20+'px'}">
            ...
            <el-col class="col" :span="22">
              <el-input size="small" style="width: 90%;" placeholder="请填写名称" v-model.trim="item.name"
                        v-if="item.name!=null && (level!=0 || type!=1)" @focus="focus(item)"
          ...
          <td style="width: 20%">
            <el-autocomplete size="small" class="inline-input" v-model="item.mock" :fetch-suggestions="querySearch"
                             placeholder="选择或者填入你的值" @mouseenter.native="focusAuto(item)"
                             @mouseleave.native="blur(item)" style="width:100%" popper-class="my-autocomplete"
                             v-if="item.type!=3 && item.type!=4 && (item.value && (item.value.data.length>0 || item.value.status))">
            ...
            <el-input size="small" placeholder="请填写值" v-model="item.mock"
                      v-else-if="item.type!=3 && item.type!=4 && (!item.value || (item.value.data.length==0 && !item.value.status))"
                      @focus="focus(item)" @blur="blur(item)"></el-input>
          </td>
          <td style="width: 5%">
          ...
          <td style="width: 5%">
            <el-button type="text" style="font-size: 15px" icon="el-icon-plus" size="mini" @click="add(arr)"
