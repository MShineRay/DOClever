<template>
  <el-row class="row" style="cursor: pointer;height: 100%">
    <table width="100%"
           style="border-spacing: 0;border-collapse: collapse;table-layout:fixed;word-wrap:break-word;word-break: break-all;"
           :style="{backgroundColor:'rgb(255,255,'+(255-level*20)+')'}">
      <tr v-if="level==0"
          style="text-align: center;vertical-align: middle;cursor: move;height: 40px;line-height: 40px;color: gray">
        <td style="width: 30%">
          名称
        </td>
        <td style="width: 14%">
      ...
      <template v-for="(item,index) in arr">
        <tr style="text-align: center;vertical-align: middle;cursor: pointer;height: 40px;line-height: 40px">
          <td style="width: 30%;vertical-align: middle">
            <el-row class="row" :style="{paddingLeft:level*20+'px'}" @click.native="toggle(item)">
              <el-col class="col" :span="2" v-if="(item.type==4 || item.type==3)"
                      :style="{borderLeft:level>0?'1px lightgray dashed':'none'}">
          ...
          <td style="width: 20%">
            <el-autocomplete size="small" v-model="item.mock" :fetch-suggestions="querySearch" placeholder="选择参考值"
                             @mouseenter.native="focusAuto(item)" style="width:100%" popper-class="my-autocomplete"
        ...
        <tr v-if="(item.type==4 || item.type==3) && item.show">
          <td colspan="5" style="width: 100%;margin:0;padding: 0 ">
            <bodyjsonpreview :source="item.data" :le="level+1" :parent="item" :index="index"
                             :data="data"></bodyjsonpreview>
          </td>
        </tr>
      </template>
    </table>
