        ...
        <tr style="text-align: center;vertical-align: middle">
          ...
          <td style="width: 5%">
            <el-popover trigger="hover" placement="bottom" width="200"
              ...
              <el-button slot="reference" size="mini" type="text" style="font-size: 13px" @click="configValue(item)">
                {{(item.value && (item.value.data.length > 0 || item.value.status)) ? "已填值" : "未填值"}}
              </el-button>
            </el-popover>
          </td>
          <td style="width: 10%">
            <el-button size="mini" type="text" style="color: red;font-size: 15px" @click="remove(index)"
                       icon="el-icon-close" v-if="index!=arr.length-1"></el-button>
          </td>
        </tr>
      </template>
    </table>
