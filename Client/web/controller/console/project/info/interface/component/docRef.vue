    ...
    <el-row class="row" style="height: 100%;border-top: 1px lightgray solid;">
      <el-col class="col" :span="5" style="overflow-y: auto;border-right: 1px lightgray solid;" id="docRefList">
        <template v-for="item in arrList">
          <el-row class="row docRefListHover"
                  style="height: 35px;line-height: 35px;text-align: center;cursor: pointer;border-bottom: 1px solid lightgray"
                  :style="{backgroundColor:item.select?'#f3f3f3':'white'}" @click.native="info(item)">
            {{item.name}}
          </el-row>
        </template>
      </el-col>
      <el-col class="col docPreview" :span="19" v-if="obj._id">
        ...
        <el-row class="row"
                style="height: 30px;line-height: 30px;border-bottom: 1px lightgray solid;padding-left: 10px">
          创建者:{{obj.owner.name}}&nbsp;&nbsp;创建时间:{{obj.createdAt}}
        </el-row>
        <el-row class="row"
                style="height: calc(100% - 60px);overflow-y: auto;word-break: break-all;padding: 0 10px 0 10px;line-height: 1.5"
                v-html="preview" id="preview">
