<template>
  <el-row class="row" style="font-size: 14px" id="paramContent">
    ...
    <expand ref="query" :expand="queryAutoExpand">
      ...
      <el-button size="mini" type="primary" style="margin-top: 5px;margin-left: 10px" @click="importQuery">导入Query字符串
      </el-button>
    </expand>
    <expand ref="header">
      <div slot="title">{{headerTab}}</div>
      <inparamheader :index="index" :item="item"></inparamheader>
      <el-button size="mini" type="primary" style="margin-top: 5px;margin-left: 10px" @click="importHeader">
        导入HTTP Header字符串
      </el-button>
    </expand>
    <expand v-if="interfaceEdit.method=='POST' || interfaceEdit.method=='PUT' || interfaceEdit.method=='PATCH'"
            ref="body" :expand="bodyAutoExpand">
      <div slot="title">{{bodyTab}}</div>
      <inparambody :index="index" :item="item"></inparambody>
      <el-button size="mini" type="primary" style="margin-top: 5px;margin-left: 10px" @click="importBody"
                 v-if="bodyInfo.type==0">导入Body字符串
      </el-button>
    </expand>
    <expand ref="inject">
      ...
      <el-row class="row" style="background-color: white;margin-top: 15px;">
        ...
        <el-row class="row" style="height: 60px;line-height: 60px;" v-else>
          <el-col class="col" :span="14" style="height: 100%;text-align: center">
            <el-tooltip class="item" effect="dark" :content="outInfo.rawRemark" placement="bottom"
                        :disabled="!outInfo.rawRemark">
          ...
          <el-col class="col" :span="10" style="text-align: center">
            <el-tooltip class="item" effect="dark" :content="outInfo.rawMock" placement="bottom"
                        :disabled="!outInfo.rawMock">
        ...
        <el-button type="primary" size="mini" style="margin-top: 5px;margin-left: 20px" @click="importJSON"
                   v-if="outInfo.type==0">
          导入JSON
        </el-button>
        <span style="right: 50px;font-weight: bold;top: 7px;position: absolute"><a
          href="/html/web/resource/other/mockrule.html" style="color: #50a3ff;" target="_blank">mock规则</a></span>
      </el-row>
    </expand>
  </el-row>
</template>
