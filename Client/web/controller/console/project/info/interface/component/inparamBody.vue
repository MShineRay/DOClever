<template>
  <el-row class="row">
        ...
        <tr style="text-align: center;vertical-align: middle">
          <td style="width: 20%;text-align: center;vertical-align: middle;height: 50px">
            <el-input size="small" style="width: 90%" placeholder="请填写参数名称" v-model.trim="item.name"
                      @input="index==arr.length-1?arr.push({name:'',type:0,must:0,remark:''}):''"></el-input>
          </td>
          ...
          <td style="width: 5%">
            <el-popover trigger="hover" placement="bottom" width="200"
              ...
              <el-button slot="reference" type="text" size="mini" @click="configValue(item)" style="font-size: 13px">
                {{(item.value && (item.value.data.length > 0 || item.value.status)) ? "已填值" : "未填值"}}
              </el-button>
            </el-popover>
          </td>
          <td style="width: 5%">
            <el-button type="text" style="color: red;font-size: 15px" size="mini" icon="el-icon-close"
    ...
    <el-row class="row" v-else-if="info.type==1 && info.rawType==2">
      ...
      <el-button type="primary" size="mini" style="margin-top: 5px;margin-left: 20px" @click="importJSON">导入JSON
      </el-button>
    </el-row>
    <el-row class="row" style="height: 50px;line-height: 50px" v-else>
      <el-col class="col" :span="20" style="text-align: center">
        <el-input size="small" style="width: 90%;" placeholder="请填写备注" v-model="info.rawTextRemark"
                  v-if="info.rawType==0"></el-input>
      ...
      <el-col class="col" :span="4" style="text-align: center">
        <el-button type="text" size="mini" @click="configRawValue" v-if="info.rawType==0" style="font-size: 13px">
          {{info.rawText ? "已填值" : "未填值"}}
        </el-button>
      </el-col>
    </el-row>
  </el-row>
