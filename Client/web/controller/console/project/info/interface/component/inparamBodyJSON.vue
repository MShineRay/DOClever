    ...
    <table width="100%" style="border-spacing: 0;border-collapse: collapse">
      <template v-for="(item,index) in arr">
        <tr style="text-align: center;vertical-align: middle;cursor: move;height: 50px;line-height: 50px"
            :draggable="item.drag?item.drag:'false'" @dragover="dragOver($event,item)"
            @dragleave="dragLeave($event,item)" @dragenter="dragEnter($event,item)" @drop="drop($event,item,index,arr)"
            @dragstart="dragStart($event,item,index,arr)" @dragend="dragEnd($event)">
          <td :style="{width: '30%',verticalAlign: 'middle',paddingLeft:level*20+'px'}">
            ...
            <el-col class="col" :span="22">
              <el-input size="small" style="width: 90%;"
                        :placeholder="(item.name!=null && (level!=0 || type!=1))?'请填写名称':'该字段没有名称'"
                        v-model.trim="item.name" @focus="focus(item)" @blur="blur(item)"
                        :disabled="(item.name!=null && (level!=0 || type!=1))?false:true">
          ...
          <td style="width: 28%">
            <el-tooltip class="item" effect="dark" :content="item.remark" placement="bottom" :disabled="!item.remark">
              <el-input size="small" style="width: 90%;height: 46px;line-height: 46px" placeholder="请填写备注;"
                        v-model="item.remark" @focus="focus(item)" @blur="blur(item)">
                <i slot="suffix" class="el-input__icon el-icon-edit" @click="editRemark(item)"
          ...
          <td style="width: 10%">
            <el-popover trigger="hover" placement="bottom" width="200"
                        :disabled="!(item.value && (item.value.data.length>0 || item.value.status))"
                        v-if="item.type!=3 && item.type!=4">
              <div style="width: 200px;overflow: auto;text-align: center"
                   v-if="item.value && (item.value.data.length>0 || item.value.status)">
              ...
              <el-button slot="reference" size="mini" type="text" @click="configValue(item)" style="font-size: 13px">
                {{(item.value && (item.value.data.length > 0 || item.value.status)) ? "已填值" : "未填值"}}
              </el-button>
            </el-popover>
          </td>
          <td style="width: 5%">
            <el-button size="mini" type="text" icon="el-icon-close" style="color: red;font-size: 15px"
                       @click="remove(item,index,level)"></el-button>
          </td>
          <td style="width: 5%">
            <el-button size="mini" type="text" style="font-size: 15px" icon="el-icon-plus" @click="add(arr,index)"
                       v-if="(item.type==0 || item.type==1 || item.type==2 || item.type==5)"></el-button>
            <el-dropdown v-else>
        ...
        <tr v-if="(item.type==4 || item.type==3) && (item.data && item.data.length>0) && item.show">
          <td colspan="7" style="width: 100%;margin:0;padding: 0 ">
            <inparambodyjson :source="item.data" :le="level+1" :parent="item" :index="index"
                             :data="data"></inparambodyjson>
          </td>
        </tr>
      </template>
    </table>
