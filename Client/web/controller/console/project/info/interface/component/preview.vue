  ...
  <el-row class="row" id="mainPreview">
    <el-row class="row" style="height: 35px;line-height: 35px">
      <el-button type="primary" size="mini" style="float: right;margin-top: 4px;margin-right: 5px;margin-left: 0px"
                 v-if="interfaceEditRole" @click="$store.dispatch('changeType','edit')">
        返回
      </el-button>
      <el-button type="primary" size="mini" style="float: right;margin-top: 4px;margin-right: 5px;margin-left: 0px"
                 v-else @click="$store.dispatch('changeType','run')">
        运行
      </el-button>
    ...
    <el-row class="row"
            style="margin-top: 5px;overflow-y: auto;height: calc(100vh - 150px);padding-bottom: 80px;background-color: white;border-radius: 5px;font-size: 14px;">
      ...
      <el-tabs style="margin-top: 20px" v-model="tabIndex" id="previewParam">
        <template v-for="(item, index) in arrParam">
          <el-tab-pane :key="item.id" :label="item.name" :name="index">
            ...
            <expand
              v-if="(interfaceEdit.method=='PUT' || interfaceEdit.method=='POST' || interfaceEdit.method=='PATCH') && (bodySave.length>0 || bodyInfo.type==1)"
              ref="body">
                  ...
                  <el-row class="row" style="margin-top: 10px">
                    <div v-show="!bodyJSONType">
                      <template v-for="item in rawJSON">
                        <div class="row"
                             style="font-size: 14px;min-height: 25px;line-height: 25px;margin: 0;padding: 0;background-color: #fff9e6;word-break: break-all"
                             v-html="item">
                        </div>
                      </template>
                    </div>
