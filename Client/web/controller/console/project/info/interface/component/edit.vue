  ...
  <el-row class="row">
    <el-row class="row" style="height: 35px;line-height: 35px">
      <el-tooltip class="item" effect="dark" content="启用自动保存后，每5秒自动保存当前接口编辑信息" placement="bottom">
        <el-checkbox v-model="$store.state.autoSave" :true-label="1" :false-label="0" style="font-size: 14px">自动保存
        </el-checkbox>
      </el-tooltip>
      <el-button size="mini" type="text" icon="fa fa-arrows-alt" style="margin-left: 5px;font-size: 15px" title="放大/缩小"
                 @click="$store.getters.event.$emit('toggleMax')"></el-button>
      <el-button size="mini" type="text" icon="el-icon-document" style="margin-left: 5px;font-size: 15px" title="文档引用"
                 @click="docRef" v-if="interfaceEdit._id"></el-button>
      <el-button type="primary" size="mini" style="float: right;margin-top: 4px;margin-right: 5px;margin-left: 0px"
                 @click="$store.dispatch('changeType','run')" v-if="arrExample.length==0">
        运行
      </el-button>
      <el-dropdown split-button type="primary" size="mini" style="float: right;margin-right: 5px;margin-left: 0px"
                   @click="$store.dispatch('changeType','run')" v-else>
        运行
      ...
      <el-dropdown split-button type="primary" size="mini" style="float: right;margin-right: 5px;margin-left: 0px"
                   @click="save" v-if="interfaceEditRole" id="btnSave" :loading="savePending" :disabled="savePending">
      ...
      <el-button size="mini" style="float: right;margin-top: 4px;margin-right: 5px;margin-left: 0"
                 @click="$store.dispatch('changeType','preview')">
        预览
      </el-button>
      <transition name="el-fade-in-linear">
        <i class="fa fa-envelope-o" id="mail" v-show="mailShow"
           style="float: right;margin-top: 9px;color:#17B9E6;margin-right: 5px;cursor: pointer;width: auto "
           @click="sendMail"></i>
      </transition>
    </el-row>
    <el-row class="row" style="margin-top: 5px;overflow-y: auto;height: calc(100vh - 150px);padding-bottom: 80px">
      <el-row class="row"
              style="background-color: white;border-radius: 5px;box-shadow: 0 2px 4px 0 rgba(0,0,0,.12), 0 0 6px 0 rgba(0,0,0,.04)">
        ...
        <el-form label-position="top" label-width="80px" style="padding: 10px 100px 20px 10px" id="interfaceBasicInfo">
          <el-row class="row">
            ...
            <el-col class="col" :span="12">
              <el-form-item label="简介">
                <el-tooltip class="item" effect="dark" :content="interfaceEdit.remark" placement="bottom"
                            :disabled="!interfaceEdit.remark">
                  <el-input style="width: 90%" size="small" v-model="interfaceEdit.remark">
                    <i slot="suffix" class="el-input__icon el-icon-edit" @click="editRemark"
          ...
          <el-row class="row">
            <el-col class="col" :span="12">
              <el-form-item label="分组">
                <el-cascader expand-trigger="hover" :options="arrGroup" :show-all-levels="false"
                             style="width: 90%;text-align: center" v-model="group" :disabled="objSnapshot.id"
          ...
          <el-row class="row">
            <el-form-item label="路径">
              <el-select style="width: 20%;text-align: center" v-model="interfaceEdit.method" @input="changeMethod"
                         size="small">
              ...
              <el-input size="small" style="width: calc(75% - 14px);margin-left: 10px" placeholder="请输入接口路径(不包含BaseUrl)"
                        v-model.trim="interfaceEdit.url" @input="changeUrl" @paste.native="paste"></el-input>
            </el-form-item>
          </el-row>
          <el-row class="row" v-if="interfaceEdit.id">
            <el-form-item label="分享">
              <el-input size="small" style="width: 95%" v-model="shareUrl" id="shareUrl" disabled>
                <i slot="suffix" class="el-input__icon fa fa-copy" @click="copyClipboard" title="复制"
                   style="cursor: pointer"></i>
              </el-input>
            </el-form-item>
          </el-row>
      ...
      <el-row class="row"
              style="background-color: white;margin-top: 15px;border-radius: 5px;box-shadow: 0 2px 4px 0 rgba(0,0,0,.12), 0 0 6px 0 rgba(0,0,0,.04);"
              v-if="interfaceEdit._id">
        <el-row class="row" style="height:40px;line-height: 40px;padding-left: 10px;font-size: 14px;color: #17B9E6">
          快照信息
          <template v-if="objSnapshot.id">
            <el-button @click="returnMaster" type="primary" size="mini"
                       style="float: right;margin-right: 5px;margin-left: 0;margin-top: 6px" key="returnMaster">返回主干
            </el-button>
            <el-button @click.native="snapshotList" type="primary" size="mini"
                       style="float: right;margin-right: 5px;margin-left: 0;margin-top: 6px" key="snapshotList">列表
            </el-button>
            <el-button @click.native="rollSnapshot" type="primary" size="mini"
                       style="float: right;margin-right: 5px;margin-left: 0;margin-top: 6px" key="rollSnapshot">回滚
            </el-button>
            <el-button @click.native="removeSnapshot" type="danger" size="mini"
                       style="float: right;margin-right: 5px;margin-left: 0;margin-top: 6px" key="removeSnapshot">删除
            </el-button>
          </template>
          <template v-else>
            <el-button @click.native="snapshotList" type="primary" size="mini"
                       style="float: right;margin-right: 5px;margin-left: 0;margin-top: 6px" key="snapshotList1">列表
            </el-button>
            <el-button @click.native="createSnapshot" type="primary" size="mini"
                       style="float: right;margin-right: 5px;margin-left: 0;margin-top: 6px" key="createSnapshot">创建
            </el-button>
          </template>
        </el-row>
        <el-row class="row" style="height: 1px;background-color: lightgray"></el-row>
      ...
      <el-tabs type="border-card" editable @edit="editTab"
               style="background-color: white;padding: 0px;margin-top: 15px;border-radius: 5px;" id="mainParam"
               v-model="tabIndex">
        <template v-for="(item, index) in param">
          <el-tab-pane :key="item.id" :name="index">
                    <span slot="label">
                        <el-tooltip class="item" effect="dark" placement="bottom" width="200" :content="item.remark"
                                    v-if="item.remark">
                            <span>{{item.name}}</span>
                        </el-tooltip>
