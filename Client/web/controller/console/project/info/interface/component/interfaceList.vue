<template>
  <el-row class="row" style="cursor: pointer;white-space: nowrap;background-color: white" id="tree">
    <template v-for="(item,index) in arr">
      <el-row class="row" :draggable="interfaceEditRole && ((item.type==0 && session.sort!=2) || session.sort==2)"
              style="height: 35px;line-height: 35px;white-space: nowrap" :id="item.type==1?'recycle':('group'+index)"
              ...
              @dragend.native="dragEnd($event)" :key="item._id" v-if="level==0 || (item.data && parent.show)"
              @mouseenter.native="mouseEnter($event,item)" @mouseleave.native="mouseLeave($event,item)"
        ...
        <el-col class="col" :span="4" style="text-align: center;white-space: nowrap"
        ...
        <el-col class="col" :span="20-2*level"
                :style="{margin: 0,fontSize: '14px',color: item.type==0?'gray':'red',whiteSpace: 'nowrap',padding: 0,textOverflow:'ellipsis',overflow:'hidden',textDecoration:item.delete?'line-through':'none'}"
                @click.native="item.show=!item.show" :title="item.name">
          {{item.name}}({{item.data.length}})
        </el-col>
        <div class="col"
             style="height: 35px;white-space: nowrap;text-align: center;position: absolute;top: 0px;right: 0px;"
             v-show="item.menu && interfaceEditRole && !search">
          <el-dropdown style="height: 100%;cursor: pointer;float: right;margin-right: 3px">
            <div class="el-dropdown-link">
              <i class="el-icon-more"
                 style="border: 1px rgba(226, 226, 226, 0.71) solid;font-size: 12px;padding: 3px;color:#17B9E6;background-color: white;font-weight: 900 "></i>
            </div>
            <el-dropdown-menu slot="dropdown">
              <el-dropdown-item v-if="item.type==0">
                <div @click="addGroup(item)">新建分组</div>
              </el-dropdown-item>
              <el-dropdown-item>
                <div @click="refresh">刷新</div>
              </el-dropdown-item>
              <el-dropdown-item v-if="item.type==0">
                <div @click="renameGroup(item)">重命名</div>
              </el-dropdown-item>
              <el-dropdown-item v-if="item.type==1">
                <div @click="clear">清空</div>
              </el-dropdown-item>
              <el-dropdown-item v-if="item.type==0 && objCopy">
                <div @click="paste(item)">粘贴</div>
              </el-dropdown-item>
              <el-dropdown-item v-if="item.type==0">
                <div @click="importInterface(item)">导入接口</div>
              </el-dropdown-item>
              <el-dropdown-item v-if="item.type==0">
                <div @click="exportGroup(item)">导出分组</div>
              </el-dropdown-item>
              <el-dropdown-item v-if="item.type==0">
                <div @click="importGroup(item)">导入分组</div>
              </el-dropdown-item>
              <el-dropdown-item v-if="item.type==0 && item.delete">
                <div @click="mergeGroup(item)">合并</div>
              </el-dropdown-item>
            </el-dropdown-menu>
          </el-dropdown>
          <div style="height: 35px;line-height: 35px;display: inline-block;margin-right: 3px;float: right;"
            ...
            <i class="el-icon-delete"
               style="border: 1px rgba(226, 226, 226, 0.71) solid;font-size: 12px;padding: 3px;color:red;background-color: white;font-weight: 900"
               v-if="item.type==0" @click="removeGroup(item)" title="删除分组"></i>
          </div>
          <template v-if="interfaceEditRole && item.type==0">
            <div style="height: 35px;line-height: 35px;display: inline-block;margin-right: 3px;float: right;"
                 v-if="template.length==0">
              <i class="el-icon-plus"
                 style="border: 1px rgba(226, 226, 226, 0.71) solid;font-size: 12px;padding: 3px;color:#17B9E6;background-color: white;font-weight: 900"
                 @click="addInterface(item)" title="新建接口"></i>
            </div>
            <el-dropdown style="height: 100%;cursor: pointer;float: right;margin-right: 3px" v-else>
              <div class="el-dropdown-link"
                   style="height: 35px;line-height: 35px;display: inline-block;margin-right: 3px;float: right;">
                <i class="el-icon-plus"
                   style="border: 1px rgba(226, 226, 226, 0.71) solid;font-size: 12px;padding: 3px;color:#17B9E6;background-color: white;font-weight: 900"
                   title="新建接口"></i>
              </div>
              <el-dropdown-menu slot="dropdown">
                <el-dropdown-item>
                  <div @click="addInterface(item)">新接口</div>
                </el-dropdown-item>
                <el-dropdown-item>
                  <div @click="addInterfaceFromTemplate(item)">从模板创建</div>
                </el-dropdown-item>
              </el-dropdown-menu>
            </el-dropdown>
          </template>
      ...
      <el-collapse-transition>
        <interfacelist v-if="item.data && item.data.length>0 && item.show" :level="level+1" :data="item.data"
                       :parent="item"></interfacelist>
      </el-collapse-transition>
      <el-row class="row" :draggable="interfaceEditRole" style="height: 35px;line-height: 35px;cursor: move"
              @mouseenter.native="mouseEnter($event,item)" @mouseleave.native="mouseLeave($event,item)"
              @click.native="info(item,index,$event)" :section="index" :row="index"
              :style="{backgroundColor:item.select?'#50bfff':(item.menu?'rgb(247,246,242':'')}"
              @dragstart.native="dragStart($event,item,index)" @dragenter.native="dragEnter($event,item)"
              @dragover.native="dragOver($event,item)" @dragleave.native="dragLeave($event)"
              @drop.native="drop($event,item,index)" @dragend.native="dragEnd($event)" :key="item._id"
              v-if="!item.data && parent.show">
        <template v-if="level>0">
          <el-col :span="2" class="col" v-for="n in level" :style="{'borderRight':'1px lightgray dashed'}">
            &nbsp;
          </el-col>
        ...
        <el-col class="col" :span="4"
                :style="{fontSize: '13px',margin: 0,color:item.select?'white':methodColor(item.finish),padding:0,lineHeight:'35px','textAlign':'center',textDecoration:(parent.type==0 && item.delete)?'line-through':'none'}"
                name="treeMethod">
          {{item.method == "DELETE" ? "DEL" : item.method}}
        </el-col>
        <el-col class="col" :span="20-2*level"
                :style="{fontSize:'14px',margin: 0,color:item.select?'white':'gray',lineHeight:'35px',textOverflow:'ellipsis',overflow:'hidden',textDecoration:item.delete?'line-through':'none'}"
                name="treeName" :title="item.name">
          {{item.name}}
        </el-col>
        <div class="col"
             style="margin: 0;height: 35px;white-space: nowrap;text-align: center;position: absolute;top: 0px;right: 0px;width: 40px"
             v-show="item.menu">
          <el-dropdown v-if="interfaceEditRole" style="width: 100%;height: 100%;cursor: pointer">
            <div class="el-dropdown-link">
              <i class="el-icon-more"
                 style="border: 1px lightgray solid;font-size: 12px;padding: 3px;color:#17B9E6;background-color: white;font-weight: 900 "></i>
            </div>
            <el-dropdown-menu slot="dropdown">
              <el-dropdown-item v-if="parent.type==0">
                <div @click="removeInterface(item)">删除</div>
              </el-dropdown-item>
              <el-dropdown-item>
                <div @click="copy(item)">复制</div>
              </el-dropdown-item>
              <el-dropdown-item v-if="parent.type==1">
                <div @click="destroyInterface(item)">彻底删除</div>
              </el-dropdown-item>
              <el-dropdown-item>
                <div @click="exportInterface(item)">导出接口</div>
              </el-dropdown-item>
              <el-dropdown-item v-if="parent.type==0 && item.delete">
                <div @click="mergeInterface(item)">合并</div>
              </el-dropdown-item>
            </el-dropdown-menu>
          </el-dropdown>
        </div>
