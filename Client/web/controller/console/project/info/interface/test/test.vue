<template>
  <el-dialog title="快速测试" width="80%" ref="box" :visible.sync="showDialog" append-to-body>
    <el-row class="row">
      <el-form label-position="top" label-width="80px" style="padding: 0 100px 10px 10px" id="testBasicInfo"
               v-if="testType==1">
        <el-row class="row">
          ...
          <el-col class="col" :span="12">
            <el-form-item label="BaseUrl">
              <el-select size="small" style="width: 90%;" v-model="$store.state.baseUrl">
                <el-option v-for="item in baseUrls" :value="item.url" style="height: auto" :key="item.url">
                  <span>{{item.url}}</span><br><span style="font-size: 13px;color: gray">{{item.remark}}</span>
                </el-option>
              </el-select>
            </el-form-item>
          </el-col>
        ...
        <el-row class="row">
          <el-col class="col" :span="12">
            <el-form-item label="业务">
              <el-cascader size="small" expand-trigger="hover" :options="arrGroup" v-model="$store.state.groupModel"
                           style="width: 90%" :props="defaultProps"></el-cascader>
            </el-form-item>
          </el-col>
          <el-col class="col" :span="12">
        ...
        <el-row class="row">
          <el-form-item label="备注">
            <el-input size="small" type="textarea" :rows="3" style="width: 95%;vertical-align: middle"
                      placeholder="请输入关于该用例的备注" v-model="test.remark"></el-input>
          </el-form-item>
        </el-row>
      </el-form>
      <el-row class="row" v-else>
        <el-form label-width="100px">
          <el-form-item label="用例">
            <el-cascader size="small" expand-trigger="hover" :options="arrTest" v-model="arrSelTest" style="width: 90%"
                         :props="defaultProps" @change="changeTest" filterable clearable></el-cascader>
          </el-form-item>
          <el-form-item label="BaseUrl">
            <el-select size="small" style="width: 90%;" v-model="$store.state.baseUrl">
              <el-option v-for="item in baseUrls" :value="item.url" style="height: auto" :key="item.url">
      ...
      <el-row class="row" style="background-color: white;padding-bottom: 30px;">
        <el-row class="row" style="height:40px;line-height: 40px;padding-left: 10px;font-size: 14px;color: #17B9E6">
          <el-switch v-model="type" active-value="code" inactive-value="ui" active-text="代码模式" inactive-text="UI模式"
                     style="margin-top: 10px;">
          </el-switch>
          <el-button type="text" size="mini" style="float: right;margin-top: 15px"
                     :icon="interfacePending?'el-icon-loading':'el-icon-check'">
      ...
      <el-row class="row" style="background-color: white;">
        ...
        <el-row class="row" style="height: 1px;background-color: lightgray"></el-row>
        <el-row class="row" v-html="test.output" style="padding: 0px 20px;word-break: break-all;word-wrap: break-word;"
                id="testOutput">
