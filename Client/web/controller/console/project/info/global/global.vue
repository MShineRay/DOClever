        ...
        <table class="table-hover" style="width: 100%;border-collapse: collapse;">
          <template v-for="(item,index) in status">
            <tr style="height: 40px;">
              ...
              <td style="width: 10%">
                <el-button type="text" size="small" style="font-size: 15px;" title="导出" icon="el-icon-upload2"
                           @click="exportJSON(item)">
                </el-button>
              </td>
              <td style="width: 10%">
                <el-button type="text" size="small" style="color: red;font-size: 15px;" icon="el-icon-close"
                           @click="remove(item,index)" title="删除" v-if="globalStatusRole"></el-button>
              </td>
            </tr>
          </template>
        </table>
        <el-button size="mini" type="primary" style="margin-top: 15px;margin-right: 10px" @click="createStatus"
                   v-if="globalStatusRole">新建状态码
        </el-button>
        <el-button size="mini" type="primary" style="margin-top: 15px;" @click="importJSON" v-if="globalStatusRole">导入
        </el-button>
      </el-row>
    </expand>
    <expand class="row"
        ...
        <table class="table-hover" style="width: 100%;border-collapse: collapse;" id="templateList">
          <template v-for="(item,index) in template">
            <tr style="height: 40px;">
              ...
              <td style="width: 10%">
                <el-button type="text" size="mini" style="color: red;" @click="removeTemplate(item,index)"
                           v-if="globalTemplateRole">
                  删除
                </el-button>
              </td>
            </tr>
        ...
        <template v-for="item in arrArticle">
          <el-row class="row article" @click.native="editArticle(item,index)" style="cursor: pointer">
            ...
            <el-row class="row" style="color: gray;font-size: 13px">
              {{item.updatedAt}}&nbsp;&nbsp;&nbsp;
              <el-button type="text" size="small" style="color:#FF4949" icon="el-icon-close"
                         @click.stop="removeArticle(item,index)" titile="删除" v-if="globalDocRole">
              </el-button>
            </el-row>
          </el-row>
        </template>
