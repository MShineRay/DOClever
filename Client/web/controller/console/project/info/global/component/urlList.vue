    ...
    <table width="100%">
      <template v-for="(item,index) in arr">
        <tr style="vertical-align: middle;height: 40px">
          <td style="width: 50%">
            <el-input size="small" style="width: 90%;margin: 0 auto" placeholder="请填写BaseUrl地址"
                      v-model="item.url"></el-input>
          </td>
          <td style="width: 30%">
            <el-input size="small" style="width: 90%;margin: 0 auto" placeholder="请填写环境变量备注"
                      v-model="item.remark"></el-input>
          </td>
          <td style="width: 10%">
            <el-button type="text" size="mini" style="font-size: 15px;" @click="edit(item)" icon="el-icon-edit"
                       title="修改变量"></el-button>
          </td>
          <td style="width: 10%">
            <el-button type="text" size="mini" style="color: red;font-size: 15px;" @click="remove(index)"
                       icon="el-icon-close" v-if="globalBaseUrlRole"></el-button>
          </td>
        </tr>
      </template>
