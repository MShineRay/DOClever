...
"></i>
    ...
    <el-row class="row" style="margin-top: 10px;padding: 0 10px;">
      ...
      <el-row class="row"
              style="height: 100px;border: 1px dashed lightgray;border-top:none;text-align: center;line-height: 100px;;font-size: 20px;color: gray"
              v-if="arr.length==0">
        请点击上方按钮添加对象
      </el-row>
      <el-row class="row" style="max-height: 300px;overflow-y: auto" id="testUIEdit" v-else>
        <el-row class="row testUILine" v-for="(item,index) in arr"
                style="height: 40px;line-height: 40px;text-align: center;cursor: move;" draggable="true"
          ...
          <el-col class="col" :span="3" v-html="getTypeName(item.type,item)"
                  :style="{textDecoration:item.modify==2?'line-through':(item.modify==1?'underline':'none'),textDecorationColor:item.modify==1?'red':'black'}">
          </el-col>
