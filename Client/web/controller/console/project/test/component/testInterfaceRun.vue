  ...
  <el-dialog title="编辑接口" :visible.sync="showDialog" width="80%" ref="box" :close-on-click-modal="false" append-to-body>
    <el-row class="row" style="height: 50px;line-height: 50px;text-align: center">
      ...
      <el-col class="col" :span="10">
        <el-cascader size="small" expand-trigger="hover" :options="arrInterface" v-model="arrSelInterface"
                     style="width: 90%" :show-all-levels="false" filterable clearable @change="changeInterface"
                     placeholder="请选择接口" :props="props">
        </el-cascader>
      </el-col>
      <el-col class="col" :span="2">
        BaseUrl
      ...
      <el-col class="col" :span="10">
        <el-select size="small" style="width: 90%;text-align: center" v-model="baseUrl">
          ...
          <el-option v-for="item in arrUrl" :value="item.url" style="height: auto" :key="item.url">
            <span>{{item.url}}</span><br><span style="font-size: 13px;color: gray">{{item.remark}}</span></el-option>
        </el-select>
      </el-col>
    </el-row>
    <el-row class="row" style="height: 50px;line-height: 50px;text-align: center">
    ...
    <el-row class="row" style="height: 60px;line-height: 60px;text-align: center">
      ...
      <el-col class="col" :span="22">
        <el-input size="small" type="textarea" :rows="2" style="width: 95%" v-model="objInterface.remark"
    ...
    <div style="position: absolute;top: 390px;right: 40px;z-index: 1000;font-size: 14px" v-if="objInterface._id">
      切换:&nbsp;
      <el-autocomplete size="mini" v-model="exampleName" :fetch-suggestions="querySearchExample" placeholder="筛选你的运行实例"
                       @select="changeExample">
      </el-autocomplete>
    </div>
    <el-tabs type="card"
             style="background-color: white;padding: 20px;margin-top: 15px;border-radius: 5px;box-shadow: 0px 2px 2px #888888;"
             v-model="tabIndex" id="mainTest">
        ...
        <el-tab-pane :key="item.id" :name="index">
          ...
          <el-row class="row">
            <el-tabs type="card">
              ...
              <el-tab-pane :label="bodyTab"
                           v-if="objInterface.method=='POST' || objInterface.method=='PUT' || objInterface.method=='PATCH'">
