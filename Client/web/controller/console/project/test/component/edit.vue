<template>
  <el-row class="row" style="padding: 0 10px 0 10px;font-size: 14px">
    <el-col class="col" :span="6" style="padding-right: 5px;height: 35px;line-height: 35px">
      <el-row class="row">
        <el-select size="mini" v-model="$store.state.selUser" :style="{width: cooperationRole?'85%':'100%'}"
                   @input="changeUser">
          <el-option v-for="item in arrUser" :label="item.name" :value="item._id"></el-option>
        </el-select>
        <el-button size="mini" icon="fa fa-users" type="text" style="width: calc(15% - 5px)" @click="editCooperation"
                   v-if="cooperationRole"></el-button>
      </el-row>
      <el-row class="row"
              style="height: calc(100vh - 155px);overflow-y: auto;margin-top: 5px;background-color: white;border-radius: 5px;box-shadow: 0 2px 4px 0 rgba(0,0,0,.12), 0 0 6px 0 rgba(0,0,0,.04)">
        ...
        <el-button style="position: absolute;right: 10px;top: 0px;width: 20px;height: 20px;font-weight: 900" type="text"
                   title="添加模块" @click="addModule" v-if="editRole">
