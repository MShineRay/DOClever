      ...
      <template v-for="(item,index) in arr">
        <tr :style="{textAlign: 'center',verticalAlign: 'middle',backgroundColor:item.enable?'white':'lightgray'}">
          <td style="width: 20%;text-align: center;vertical-align: middle;height: 50px">
            <el-input size="small" style="width: 90%;" placeholder="请填写参数名称" v-model="item.name"
          ...
          <td style="width: 20%;text-align: center;vertical-align: middle;height: 50px;line-height: 50px">
            <div style="width: 90%;display: inline-block;"
              ...
              <el-autocomplete size="small" class="inline-input" v-model="item.selValue"
                               ...
                               :disabled="!item.enable || interface.example" custom style="width:100%"
            ...
            <el-input size="small" style="width: 90%;" placeholder="请填写值" v-model="item.selValue"
                      v-else-if="item.type==0" custom :disabled="!item.enable || interface.example"></el-input>
          </td>
          <td style="width: 5%;">
          ...
          <td style="width: 10%;height: 50px;">
            <el-button type="text" size="mini" style="font-size: 13px" @click="encrypt(item)" v-if="item.type==0"
                       :disabled="interface.example">{{(item.encrypt && item.encrypt.type) ? item.encrypt.type : "未加密"}}
            </el-button>
          </td>
          <td style="width: 15%">
            <el-button type="text" size="mini" style="font-size: 15px;" @click="toggleEnable(item,index)"
                       :disabled="interface.example"><span :class="item.enable?'fa fa-eye-slash':'fa fa-eye'"
                                                           :title="item.enable?'发送时不包含此字段':'发送时包含此字段'"></span>
            </el-button>
          </td>
        </tr>
      </template>
    ...
    <el-row class="row" style="padding: 0 0 0 20px;" v-show="info.type==1 && info.rawType!=2">
      <el-row class="row" style="height: 50px;line-height: 50px;margin: 0;padding: 0" v-if="info.rawType==0">
        ...
        <el-col class="col" :span="5" style="text-align: center" v-show="info.rawType==0 && saltShow">
          <el-input size="small" placeholder="请填入salt的值" id="bodyRawEncryptSalt"
                    v-model="interface.encrypt.salt"></el-input>
        </el-col>
      </el-row>
      <el-row class="row" style="margin-top: 10px">
        <el-input size="small" type="textarea" :rows="6" style="width: 100%;height: 200px;" v-if="info.rawType==0"
