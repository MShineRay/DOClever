        ...
        <tr style="text-align: center;vertical-align: middle;cursor: move;height: 50px;line-height: 50px">
          <td :style="{width: '30%',verticalAlign: 'middle',paddingLeft:level*20+'px'}">
            ...
            <el-col class="col" :span="22">
              <el-input size="small" style="width: 90%;" placeholder="请填写名称" v-model="item.name" v-if="item.name!=null"
          ...
          <td style="width: 8%">
                        <span style="display: inline-block;">
                            <el-checkbox v-model="item.must" :true-label="1" :false-label="0"
                                         :disabled="true">必有</el-checkbox>
                        </span>
          </td>
          <td style="width: 18%">
            <el-input size="small" type="textarea" style="width: 90%;height: 46px;line-height: 46px" resize="none"
                      :rows="0" placeholder="请填写备注;" v-model="item.remark" :disabled="true"></el-input>
          </td>
          <td style="width: 30%">
            <el-autocomplete size="small" class="inline-input" v-model="item.mock" :fetch-suggestions="querySearch"
                             placeholder="选择或者填入你的值" @mouseenter.native="focusAuto(item)" style="width:100%"
                             popper-class="my-autocomplete"
                             v-if="item.value && (item.value.data.length>0 || item.value.status)"
                             :disabled="interface.example">
            ...
            <el-input size="small" placeholder="请填写值" v-model="item.mock"
                      ...
                      :disabled="interface.example"></el-input>
          </td>
        </tr>
