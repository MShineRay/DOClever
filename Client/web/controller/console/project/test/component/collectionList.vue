<template>
  <el-row class="row">
    <el-row class="row" v-for="(item,index) in arr"
            style="padding-left: 10px;line-height: 30px;height: 30px;font-size: 14px;color:#17B9E6;cursor: pointer "
            @mouseenter.native="mouseEnter($event)" @mouseleave.native="mouseLeave($event)"
            @click.native="info($event,item)">
      {{item.name}}
      <div class="collectionMenu"
           style="height:26px;width:auto;line-height: 26px;position: absolute;right: 5px;top:2px;display: none"
           @click="$event.stopPropagation()" v-if="editRole">
        <i class="el-icon-edit"
           style="border: 1px rgba(226, 226, 226, 0.71) solid;font-size: 12px;padding: 2px;color:#17B9E6;background-color: white;font-weight: 900;"
           @click="renameCollection(item)" title="修改名称"></i>
        <i class="el-icon-delete"
           ...
           @click="remove(item,index)" title="删除"></i>
      </div>
    </el-row>
  </el-row>
</template>
