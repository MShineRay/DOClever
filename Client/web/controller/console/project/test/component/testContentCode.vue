<template>
  <el-row class="row" id="testContentCode">
    <el-row class="row" style="height: 50px;line-height: 50px">
      ...
      <a href="/html/web/resource/other/test.html" style="float: right;margin-right: 20px;color: #17b9e6"
         target="_blank">如何编写</a>
    </el-row>
    <el-row class="row" style="height: 300px;border: 1px solid lightgray;margin-left: 10px;width: calc(100% - 20px)">
      <el-col class="col" style="width: 40px;background-color: lightgray;padding-top: 5px;overflow-y: hidden;"
              :style="{paddingBottom:5+scrollHeight+'px'}" id="testScroll">
        <el-row class="row" style="text-align: right;padding-right: 5px;line-height: 1.5;font-size: 15px"
                v-for="n in line">
          {{n}}
          <span style="position: absolute;left: 0px;top: 0px;color: red;width: 26px;height: 26px;text-align: left"
                v-if="objAnalyse.err.indexOf(n)>-1">
                        X
                    </span>
        </el-row>
      </el-col>
      <el-col class="col"
              style="width:calc(100% - 40px);box-sizing:padding-box;background-color: rgba(255,255,224,0.5)">
        <div class="row"
             style="margin: 0;width: 100%;height:100%;box-sizing:border-box;padding: 5px;overflow-y: auto;word-break: keep-all;outline: none;line-height: 1.5;font-size: 15px;white-space: pre"
             contenteditable="true" id="testContent" spellcheck="false" @input="changeContent($event)"
             @keydown="keyDown($event)" @scroll="scroll($event)" @keyup="keyUp($event)">

        </div>
      </el-col>
    </el-row>
    <el-row class="row"
            style="position: absolute;width: 150px;overflow-x:hidden;max-height: 200px;overflow-y: auto;background-color: white;border: 1px solid #f3f3f3;border-radius: 5px;box-shadow: 0 2px 4px 0 rgba(0,0,0,.12), 0 0 6px 0 rgba(0,0,0,.04);z-index: 1000"
            v-if="menu && vars.length>0" id="autoMenu">
      <el-row class="row autoMenu" v-for="(item,index) in vars"
              style="width: 100%;height: 30px;line-height:30px;text-align:left;padding-left: 5px;cursor: default"
              :style="{backgroundColor:selMenuIndex==index?'#22B5F3':'white',color:selMenuIndex==index?'white':'gray'}"
