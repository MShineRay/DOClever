    ...
    <tempalte v-else>
      <el-row class="row" style="height: 50px;line-height: 50px;padding-right: 20px">
        ...
        <el-button size="small" type="text" style="float: right;margin-right: 10px;margin-top: 15px;color: gray">
          <div
            style="display: inline-block;height: 10px;width: 10px;border-radius: 5px;background-color: #67C23A;"></div>&nbsp;观察者
        </el-button>
        <el-button size="small" type="text" style="float: right;margin-right: 10px;margin-top: 15px;color: gray">
          <div
            style="display: inline-block;height: 10px;width: 10px;border-radius: 5px;background-color: #17b9e6;"></div>&nbsp;管理员
        </el-button>
      </el-row>
      <el-row class="row"
              style="margin: 10px 20px 10px 20px;width: calc(100% - 40px);height: calc(100vh - 120px);overflow-y: auto">
        <expand :expand="1"
                style="background-color: white;border-radius: 5px;box-shadow: 0 2px 4px 0 rgba(0,0,0,.12), 0 0 6px 0 rgba(0,0,0,.04);"
                v-if="arrCreate.length>0">
          <div slot="title" style="font-size: 14px">
            我创建的团队
          </div>
          <el-radio-group v-model="$store.state.teamCreateSort" size="mini" slot="append" style="margin-right: 20px"
                          @change="changeCreateSort">
        ...
        <expand :expand="1"
                style="background-color: white;margin-top: 20px;border-radius: 5px;box-shadow: 0 2px 4px 0 rgba(0,0,0,.12), 0 0 6px 0 rgba(0,0,0,.04);"
                v-if="arrJoin.length>0">
          <div slot="title" style="font-size: 14px">
            我加入的团队
          </div>
          <el-radio-group v-model="$store.state.teamJoinSort" size="mini" slot="append" style="margin-right: 20px"
                          @change="changeJoinSort">
            <el-radio-button :label="0">时间&nbsp;↓</el-radio-button>
            <el-radio-button :label="1">名称&nbsp;↑</el-radio-button>
          </el-radio-group>
          <el-row class="row">
    ...
    <el-dialog title="新建团队" :visible.sync="showAdd" width="50%" append-to-body>
      <el-form label-position="top" ref="form" label-width="100px">
        ...
        <el-form-item label="描述">
          <el-input size="small" type="textarea" :rows="2" style="width: 100%" v-model="dis"
                    placeholder="请输入新团队的简介"></el-input>
        </el-form-item>
      </el-form>
    ...
    <el-dialog title="团队申请" :visible.sync="showApply" width="50%" ref="apply" append-to-body>
      <el-form ref="form" label-width="100px">
        ...
        <el-form-item label="备注">
          <el-input size="small" type="textarea" :rows="2" style="width: 80%" v-model="applyDis"
                    placeholder="请输入你要申请的备注"></el-input>
        </el-form-item>
      </el-form>
      <span slot="footer" class="dialog-footer">
