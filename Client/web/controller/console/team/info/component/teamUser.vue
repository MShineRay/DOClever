<template>
  <el-row class="row">
    <el-row class="row" style="height: 40px;line-height: 40px;">
      ...
      <el-input size="small" style="float: right;width: 200px;margin-right: 10px" placeholder="请输入你要筛选的用户名"
                v-model="searchName">
        ...
        <template v-for="(item,index) in arrFilter">
          <el-collapse-item class="hover" :key="item._id">
            <template slot="title">
              ...
              <el-button size="mini" type="danger" @click.native="removeGroup(item,index)"
                         style="float: right;margin-right: 20px;margin-top: 10px">删除
              </el-button>
              <el-button size="mini" type="primary" @click.native="renameGroup(item)"
                         style="float: right;margin-right: 5px;margin-top: 10px;margin-left: 0px">重命名
              </el-button>
              <el-button size="mini" type="primary" @click.native="inviteUser(item)"
                         style="float: right;margin-right: 5px;margin-top: 10px">邀请用户
              </el-button>
            </template>
              ...
              <el-row class="row" style="height: 40px;line-height: 40px;text-align: center" :key="item1.user._id">
                ...
                <el-col class="col" :span="6">
                  <template v-if="item1.role!=2">
                    <el-select v-if="manageRole && session.id!=item1.user._id" v-model="item1.role"
                               @input="changeRole(item1)">
                ...
                <el-col class="col" :span="4">
                  <el-button size="mini" type="text" @click.native="moveUser(item1,index1,item)"
                             v-if="(manageRole && item1.role!=2 && !ownRole) || ownRole">移动
                  </el-button>
                  <el-button size="mini" type="text" @click.native="removeUser(item1,index1,item)"
                             v-if="item1.role!=2 && ((manageRole && item1.role!=2 && !ownRole) || ownRole)"
                             style="color: red">删除
                  </el-button>
                </el-col>
              </el-row>
