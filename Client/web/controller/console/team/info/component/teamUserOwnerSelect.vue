        ...
        <template v-for="(item,index) in arrFilter">
          <el-collapse-item :title="item.name" class="hover" :key="item._id">
            <template v-for="(item1,index1) in item.users">
              <el-row class="row" style="height: 40px;line-height: 40px;text-align: center;cursor: pointer"
                      @click.native="select(item1)"
                      :style="{'backgroundColor':selectUser==item1.user._id?'lightblue':'white'}" :key="item1._id">
