  ...
  <el-dialog title="新建项目" width="80%" ref="box" :visible.sync="showDialog" append-to-body>
    <el-form ref="form" label-width="100px">
      ...
      <el-form-item label="描述" style="text-align: center">
        <el-input size="small" type="textarea" :rows="3" style="width: 80%" name="dis" v-model="obj.dis"
                  placeholder="请输入项目描述"></el-input>
      </el-form-item>
      <el-form-item label="公开" style="text-align: center" v-if="category==0 || category==1">
        <el-switch v-model="obj.public" active-color="#13ce66" inactive-color="#ff4949" :active-value="1"
                   :inactive-value="0"></el-switch>
      </el-form-item>
      <el-form-item label="所有者" style="text-align: center">
        <el-autocomplete size="small" :fetch-suggestions="querySearchAsync.bind(this,0)" style="width: 80%" name="owner"
                         v-model="obj.owner.name" placeholder="请输入所有者名称" @select="sel"></el-autocomplete>
      </el-form-item>
        ...
        <template v-for="(item,index) in obj.users">
          <el-row class="row" style="height: 50px;line-height: 50px;width: 90%;display: inline-block">
            <el-col :span="12" class="col">
              <el-autocomplete size="small" :fetch-suggestions="querySearchAsync.bind(this,index)" style="width: 80%"
                               v-model="item.name" @input="index==obj.users.length-1?add():''" placeholder="请输入用户名称"
                               @select="selUser"></el-autocomplete>
            </el-col>
