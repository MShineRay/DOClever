<template>
  <el-row class="row" style="margin:0 0 0 5px" id="body" :gutter="20">
    <el-col class="col" :span="6"
            style="min-height: 600px;background-color: white;box-shadow: 2px 2px 2px #888888;border-radius: 5px;margin: 0;padding: 0">
      <el-row class="row" style="height: 50px;background-color: #20A0FF;color: white;margin: 0;padding: 0" id="group"
              v-if="!search">
        <el-col class="col" :span="6"
                style="line-height: 50px;text-align: center;font-weight: bold;font-size: 15px;padding: 0">
          分组
        </el-col>
        <el-col class="col" :span="15">

        </el-col>
        <el-col class="col" :span="3" style="cursor: pointer;text-align: center;line-height: 50px;" title="搜索"
                @click.native="search=true">
          <i class="el-icon-search"></i>
        </el-col>
      </el-row>
      <el-row class="row" style="height: 50px;background-color: transparent;color: white;margin: 0;line-height: 50px"
              v-else>
        <el-input placeholder="请输入查找的接口" @change="searchInterface" v-model="searchText">
          <template slot="append">
            <el-button type="text" style="font-size: 14px;width: 50px;color: #20a0ff" @click="cancelSearch">取消
            </el-button>
          </template>
    ...
    <el-col class="col" :span="18" id="info">
      <el-row class="row" v-if="preview==0 && interfaceEdit">
        <el-row class="row"
                style="background-color: white;border-radius: 5px;box-shadow: 0px 2px 2px #888888;padding: 15px 0">
          <el-row class="row" style="height: 50px;line-height: 50px">
            ...
            <el-col class="col" :span="10" style="height: 50px;line-height: 50px;text-align: left">
              <el-input style="width: 90%" placeholder="请输入接口名称" v-model="interfaceEdit.name"
                        :disabled="true"></el-input>
            </el-col>
            <el-col class="col" :span="2" style="text-align: center">
              <el-popover ref="popover1" placement="bottom" title="修改信息" width="400" trigger="hover"
                          :content="editInfo">
              </el-popover>
          ...
          <el-row class="row" style="height: 50px;line-height: 50px">
            ...
            <el-col class="col" :span="10">
              <el-input style="width: 90%" placeholder="请输入接口路径(不包含BaseUrl)" v-model="interfaceEdit.url"
                        @change="changeUrl" :disabled="true"></el-input>
            </el-col>
            <el-col class="col" :span="2" style="text-align: center;color: gray">
              方法
            </el-col>
          ...
          <el-row class="row" style="height: 50px;line-height: 50px">
            ...
            <el-col class="col" :span="10" style="text-align: left">
              <el-select style="width: 90%;text-align: center" v-model="interfaceEdit.group._id">
                <el-option v-for="item in interfaceList" :value="item._id" :label="item.name"
          ...
          <el-row class="row" style="height: 90px;line-height: 90px;text-align: center">
            ...
            <el-col class="col" :span="22" style="text-align: left">
              <el-input type="textarea" :rows="3" style="width: 95%;vertical-align: middle" placeholder="请输入关于该接口的简介"
                        v-model="interfaceEdit.remark" :disabled="true"></el-input>
            </el-col>
          </el-row>
        </el-row>
        <el-tabs type="card"
                 style="background-color: white;padding: 20px;margin-top: 15px;border-radius: 5px;box-shadow: 0px 2px 2px #888888;"
                 id="mainParam" v-model="tabIndex">
          <template v-for="(item, index) in param">
            <el-tab-pane :key="item.id" :name="index">
                            <span slot="label">
                                <el-popover placement="bottom" width="200" trigger="hover" :content="item.remark"
                                            v-if="item.remark">
